...
$(document).ready(function(){
    ...
    function initialize_map() {
        ...
        if ($('.map').length) {
            var myLatLng = new google.maps.LatLng(38.9111456,-92.3138613);
            var mapOptions = {
                zoom: 7,
                center: myLatLng,
                scrollwheel: false,
    ...
    $("#contact-form").submit( function() {
        ...
        if ( !valid_email_address( $("#user-email").val() ) || $("#user-name").val().length <= 2  )  {
            ...
            $.ajax({
                url: 'http://ericdu.com/assets/js/contact.php',
                data: data_of_form,
                type: 'POST',
                ...
                error: function(errorThrown, xhr, type,status){
                    alert("errorThrown"+JSON.parse(errorThrown);
                    alert("xhr"+xhr);
                    alert("type"+type);
                    alert("status"+status);
                    alert("te1");
